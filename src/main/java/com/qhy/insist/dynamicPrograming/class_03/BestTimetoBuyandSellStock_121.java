 ...
 * @Description [Easy]   Topics: [Array] [Dynamic Programming]
 *
 *
...
public class BestTimetoBuyandSellStock_121 {
    public int maxProfit(int[] prices) {
        int maxProfitCur = 0, maxProfitSoFar = 0;
        for (int i = 1; i < prices.length; i++) {
            maxProfitCur = Math.max(0, maxProfitCur += prices[i] - prices[i-1]);
            maxProfitSoFar = Math.max(maxProfitCur, maxProfitSoFar);
        }
