 ...
 * @Description [Easy]   Topics: [Array] [Greedy]
 *
 *
...
public class BestTimetoBuyandSellStockII_122 {
    public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            if (prices[i] > prices[i-1]) {
                total = Math.max(0, total += prices[i] - prices[i-1]);
            }
        }
        return total;
    }
    ...
    public static void main(String[] args) {
        ...
        BestTimetoBuyandSellStockII_122 bestTimetoBuyandSellStock = new BestTimetoBuyandSellStockII_122();
